...
const fetchUploadedDraftFiles = async (
  ...
  const { data: draftFiles } = await axiosInstance.get<DraftFile[]>(
    '/draft-files',
    {
...
const uploadDraftFile = async (
  ...
  const { data: uploadedDraftFile } = await axiosInstance.post<DraftFile>(
    '/draft-files',
    draftFileFormData,
    {
...
const deleteDraftFile = (draftFileId: number) =>
  axiosInstance.delete(`/draft-files/${draftFileId}`)
