...
<template>
  <section class="dashboard-view" aria-labelledby="dashboard-view-title">
    <header class="dashboard-view__header">
      <div>
        <p class="dashboard-view__date">{{ formattedCurrentDate }}</p>
        <h1 id="dashboard-view-title">
          Merhaba, {{ authenticatedUserFirstName }}
        </h1>
        <p class="dashboard-view__introduction">
          Dijital işlemlerinizin özeti ve güvenli işlem araçlarınız burada.
        </p>
      </div>

      <button
        class="dashboard-view__refresh-button"
        type="button"
    ...
    <div
      ...
      class="dashboard-view__warning-notice"
      role="alert"
    >
    ...
    <div
      ...
      class="dashboard-view__loading-state"
      aria-label="Dashboard verileri yükleniyor"
      aria-busy="true"
    >
      <div class="dashboard-view__metric-skeletons">
        <span v-for="skeletonIndex in 4" :key="skeletonIndex"></span>
      </div>
      <span class="dashboard-view__feature-skeleton"></span>
      <span class="dashboard-view__table-skeleton"></span>
    </div>

    <div
      v-else-if="hasInitialDashboardError"
      class="dashboard-view__error-state"
      role="alert"
    >
      <span class="dashboard-view__error-icon" aria-hidden="true">!</span>
      <div>
    ...
    <template v-else-if="dashboardSummary">
      <section class="dashboard-view__metrics" aria-label="Hesap özeti">
        <DashboardMetricCard
...
.dashboard-view__error-state h2,
.dashboard-view__error-state p {
  margin: 0;
}
...
.dashboard-view__date::first-letter {
  text-transform: uppercase;
}
...
.dashboard-view__refresh-button,
.dashboard-view__warning-notice button,
.dashboard-view__error-state button {
  display: inline-flex;
  gap: 0.55rem;
  align-items: center;
...
.dashboard-view__refresh-button:hover:not(:disabled),
.dashboard-view__warning-notice button:hover,
.dashboard-view__error-state button:hover {
  border-color: var(--color-accent-600);
  transform: translateY(-1px);
}

.dashboard-view__refresh-button:disabled {
  cursor: wait;
  opacity: 0.65;
}

.dashboard-view__warning-notice {
  display: flex;
  gap: 1rem;
  align-items: center;
  justify-content: space-between;
...
.dashboard-view__metric-skeletons {
  display: grid;
  grid-template-columns: repeat(4, minmax(0, 1fr));
  gap: 1rem;
}

.dashboard-view__error-state {
  display: grid;
...
.dashboard-view__loading-state {
  display: grid;
  gap: 1rem;
}

.dashboard-view__metric-skeletons span,
.dashboard-view__feature-skeleton,
.dashboard-view__table-skeleton {
  display: block;
  ...
  border-radius: var(--radius-lg);
  animation: dashboard-view-shimmer 1.3s infinite linear;
}
...
@keyframes dashboard-view-shimmer {
  to {
